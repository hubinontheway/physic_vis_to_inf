...
class ImageLogger(Callback):
    """Logs validation samples: Vis | GT IR | Pred IR."""
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        with torch.no_grad():
            pred = pl_module(vis).clamp(0.0, 1.0)

        if isinstance(trainer.logger, TensorBoardLogger):
            writer = trainer.logger.experiment

            def _grid_images(v, gt, pr):
                gt_3c = gt.repeat(1, 3, 1, 1)
                pr_3c = pr.repeat(1, 3, 1, 1)
                return torch.cat([v, gt_3c, pr_3c], dim=3)

            grid = _grid_images(vis, ir, pred)
...
def main():
    ...
    parser.add_argument("--config", default="configs/vis2ir/vis2ir_etrl.yml")
    ...
    model = Vis2IRETRLPlModule(config=config)
